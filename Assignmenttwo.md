...
## Part 5
We can write code that searches through a list randomly each time. We start by generating a vector of indexes and then shuffling it using a random number generator to get a new randomized list of indexes. Then you generate a vector of data, and fill it up. Then you compare the element of the data vector at the index element of the index vector at index of i up to N. This ensures that each element is potentially inspected, but utilizing a random sequence, while keeping the index of the loop and the original data the same to output the correct index. This explanation was hard to put into words exactly, but we are essentially going to use a vector of elements made up of randomly shuffled index values, and then use those values to pull elements from the data vector and compare them to our key value, but in order of actual index from 0 to N-1. 
