...
Dijkstra's algorithm finds the shortest path from a starting vertex to all other vertices in the graph. It chooses the vertex with the current known lowest total weight (distance, price, etc) from the start and then finalizes that as teh shortest path to that vertex.  Then it keeps repeating this process, finding the shortest distances to each neighbor, and adding it to the original shortest distance, until we have all of the shortest paths from the starting vertex to all other vertices. 
...
which would result in a net distance of 1. But since it was already decided that the shortest path to the store was a distance of 4, this is the finalized distance and the shortcut gets ignored. 
